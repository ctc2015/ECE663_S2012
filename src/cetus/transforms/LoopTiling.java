...
public class LoopTiling extends AnalysisPass {
	...
	public void start() {
		...
		for (i = outer_loops.size() - 1; i >= 0; i--) {
			...
			if (loops.size() < 2) {
				...
				target_loops++;
				Statement stm = ((ForLoop) loops.get(loops.size() - 1))
						.getBody();
				List<ArrayAccess> arrays = new ArrayList<ArrayAccess>(); // Arrays
																			// in
																			// loop
																			// body
				...
				// reuseValue : 'n' bit will be located in the cache before
				// reuse
				...
				if (firstNonZero > 0 && firstNonZero < (loopSize - 1)) {
					...
					for (j = 0; j < (loopSize - firstNonZero); j++) {
						...
						tiling(loops, loops.get(firstNonZero + j * 2),
								firstNonZero, blockSize);
						numOfTiling++;
					}
		...
		System.out.println("# of reuse only in the inner-most loops : "
				+ reuse_in_innermost);
	...
	private boolean tiling(List<Loop> loops, Loop targetLoop, int newLocation,
			int blockSize) {
		blockSize = 64;
		// System.out.println("blockSize = " + blockSize);
		Expression tempVariable;
		Symbol symbol = LoopTools.getLoopIndexSymbol(targetLoop);
		tempVariable = (Expression) declareTemp((Traversable) (loops.get(0)),
				symbol.getSymbolName());

		// System.out.println("before size of loops : " + loops.size());
		targetLoop = insertNewLoop(loops, targetLoop, tempVariable, symbol,
		...
		if (oriInitExp instanceof AssignmentExpression) {
			Expression oriInitLHS = ((AssignmentExpression) oriInitExp)
					.getLHS();
			Expression oriInitRHS = ((AssignmentExpression) oriInitExp)
					.getRHS();
			AssignmentOperator oriInitOp = ((AssignmentExpression) oriInitExp)
					.getOperator();
			Expression newInitExp = new AssignmentExpression(oriInitLHS,
					oriInitOp, tempVariable);
			oriInitExp.swapWith(newInitExp);
			// System.out.println("Target : New init Exp - " + newInitExp);
			// System.out.println("Target : New init Statement - " +
			// newInitStat);
		} else
		...
		if (oriCondition instanceof BinaryExpression) {
			...
			BinaryOperator condOp = ((BinaryExpression) oriCondition)
					.getOperator();
			...
			if (symbol.getSymbolName().equals(condLHS.toString())) {
				...
				Expression exp1 = new BinaryExpression(tempVariable,
						BinaryOperator.ADD, blk);
				Expression exp2 = new BinaryExpression(exp1,
				...
				// System.out.println("Target : new Condition : " +
				// newCondition);
			} else {
		...
		ForLoop newLoop = new ForLoop(newInitStat, newCondition, oriStep,
				((ForLoop) targetLoop).getBody());
		((Statement) (targetLoop)).swapWith((Statement) newLoop);

		return true;
	}

	private Loop insertNewLoop(List<Loop> loops, Loop targetLoop,
			Expression tempVar, Symbol symbol, int newLocation, int blockSize) {
		Statement oriInitStat = ((ForLoop) targetLoop).getInitialStatement();
		Statement newInitStat = oriInitStat.clone();
		List<Traversable> oriInit = newInitStat.getChildren();
		Expression oriCondition = ((ForLoop) targetLoop).getCondition();
		...
		if (oriInitExp instanceof AssignmentExpression) {
			...
			AssignmentOperator oriInitOp = ((AssignmentExpression) oriInitExp)
					.getOperator();
			newInitExp = new AssignmentExpression(tempVar, oriInitOp,
					oriInitRHS);
			oriInitExp.swapWith(newInitExp);
		...
		newStep = new AssignmentExpression(stepLHS, AssignmentOperator.ADD,
				stepRHS);
		/*
		 * if(oriStep instanceof BinaryExpression) {
		 * System.out.println("oriStep is Binary"); Expression stepRHS =
		 * ((BinaryExpression)oriStep).getRHS(); Expression stepLHS =
		 * ((BinaryExpression)oriStep).getLHS(); BinaryOperator stepOp =
		 * ((BinaryExpression)oriStep).getOperator();
		 * 
		 * if(symbol.getSymbolName().equals(stepLHS.toString())) stepLHS =
		 * tempVar; else if(symbol.getSymbolName().equals(stepRHS.toString()))
		 * stepRHS = tempVar;
		 * 
		 * newStep = new BinaryExpression(stepLHS, stepOp, stepRHS);
		 * System.out.println("Step : " + oriStep);
		 * System.out.println("new Step : " + newStep); } else if(oriStep
		 * instanceof UnaryExpression) { System.out.println("oriStep is Unary");
		 * UnaryOperator stepOp = ((UnaryExpression)oriStep).getOperator();
		 * 
		 * newStep = new UnaryExpression(stepOp, tempVar);
		 * System.out.println("Step : " + oriStep);
		 * System.out.println("new Step : " + newStep); } else newStep =
		 * (Expression)oriStep.clone();
		 */
		Object test_st = ((Statement) (loops.get(newLocation))).clone();
		Traversable t = (Traversable) (loops.get(newLocation));

		ForLoop newLoop = new ForLoop(newInitStat, newCondition, newStep,
				(Statement) (test_st));
		...
		while (iter.hasNext()) {
			...
			if (o instanceof ForLoop
					&& ((ForLoop) o).toString().equals(targetLoop.toString()))
				return (Loop) o;
		}
		return null;
	}
	...
	private Identifier declareTemp(Traversable t, String s) {
		...
		Declaration temp_decl = new VariableDeclaration(Specifier.INT,
				temp_declarator);
	...
	private int getGroupReuseAll(LinkedList<Loop> loops,
			List<ArrayAccess> arrays, int[] reuseValue) {
		long reuseDistance;
		int i, j, k;
		...
		for (i = 0; i < loops.size(); i++) {
			for (j = 0; j < arrays.size(); j++) {
				for (k = j + 1; k < arrays.size(); k++) {
					reuseDistance = LoopTools.getReuseDistance(loops.get(i),
							arrays.get(j), arrays.get(k));
		...
		// reuseDistance = LoopTools.getReuseDistance(loops.get(0),
		// arrays.get(0), arrays.get(1));
		// System.out.println("reuse dist : " + reuseDistance);
		return 0;

	}

	private int getSelfReuseAll(LinkedList<Loop> loops,
			List<ArrayAccess> arrays, int[] reuseValue) {
		long[] kernelVector;
		int loopSize = loops.size();
		List type;
	...
	private int getTypeSize(ArrayAccess array) {
		...
		types = ((Identifier) array.getArrayName()).getSymbol()
				.getTypeSpecifiers();
	...
	private double[][] getLoopMatrix(LinkedList<Loop> loops, ArrayAccess array) {
		...
		/*
		 * System.out.println("Loop Matrix : "); for(int i = 1; i <= numIndices;
		 * i++) { System.out.println(""); for(int j = 1; j <= numLoops; j++)
